	...
	public class PoisonModule : AIModule
	{
		public override void OnUpdate()
		{
			...
			if (this.m_LastAttackTime == 0f || Time.time - this.m_LastAttackTime >= this.m_PoisoningInterval)
			{
				...
				float num3 = this.m_AI.IsVisible() ? this.m_AI.m_Params.m_AttackRange : (this.m_AI.m_Params.m_AttackRange * 2f);
		...
		private void Attack()
		{
			...
			damageInfo.m_HitDir = Player.Get().transform.position + Vector3.up * Player.Get().GetComponent<CharacterControllerProxy>().height * 0.5f - base.transform.position;
