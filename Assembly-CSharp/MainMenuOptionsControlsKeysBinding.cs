...
public class MainMenuOptionsControlsKeysBinding : MainMenuScreen, IYesNoDialogOwner, IInputsReceiver
{
	private void Awake()
	{
		...
		this.m_AcceptText = this.m_AcceptButton.GetComponentInChildren<Text>();
	...
	private void UpdateButtons()
	{
		...
		RectTransform component = this.m_DefaultButton.GetComponent<RectTransform>();
		...
		component = this.m_AcceptButton.GetComponent<RectTransform>();
		if (RectTransformUtility.RectangleContainsScreenPoint(component, screenPoint))
		{
			this.m_ActiveButton = this.m_AcceptButton.gameObject;
		}
		...
		float num = (!(this.m_ActiveButton == this.m_DefaultButton.gameObject)) ? this.m_ButtonTextStartX : this.m_SelectedButtonX;
		...
		if (this.m_ActiveButton == this.m_AcceptButton.gameObject)
		{
			color = this.m_AcceptText.color;
			color.a = 1f;
			this.m_AcceptText.color = color;
		}
