...
public class StoryTab : NotepadTab
{
	...
	public void AddStoryEvent(string element_name)
	{
		...
		List<Text> componentsDeepChild = General.GetComponentsDeepChild<Text>(transform.gameObject);
		for (int i = 0; i < componentsDeepChild.Count; i++)
		{
			componentsDeepChild[i].text = GreenHellGame.Instance.GetLocalization().Get(componentsDeepChild[i].name);
		}
